...
public static class ShoppingPriceLogic
{
    ...
    public static ShoppingItemInsight Assess(ShoppingListItem item, ShoppingItemMemory? memory)
    {
        ...
        var _ThisShop = item.InBasket && item.ShoppingTripID != null
            ? _Prices.FirstOrDefault(p => p.ShoppingListItemID == item.ShoppingListItemID && p.ShoppingTripID == item.ShoppingTripID)
            : null;
